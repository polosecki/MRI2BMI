...
def conv3x3(in_planes, out_planes, stride=1):
    """3x3 3D convolution with padding."""
...
class ResNet(nn.Module):
    """3D ResNet backbone for volumetric MRI data.

    Based on the torchvision ResNet implementation adapted for 3D convolutions.
...
class SFCN(nn.Module):
    """Simple Fully Convolutional Network (SFCN) for 3D brain MRI regression.

    Architecture from Peng et al., 2021 (https://doi.org/10.1016/j.media.2021.102013).
    The model outputs a log-probability distribution over binned target values,
    allowing soft-label KL-divergence training.
    """
